...
final class ActivateTerminalHandlerTest extends TestCase
{
    private InMemoryEventStore $eventStore;
    private InMemoryTerminalRepository $terminalRepository;
    private ActivateTerminalHandler $handler;

    protected function setUp(): void
    {
        $this->eventStore = new InMemoryEventStore();
        $this->terminalRepository = new InMemoryTerminalRepository($this->eventStore);
        $this->handler = new ActivateTerminalHandler($this->terminalRepository);
    }

    public function test_activates_a_disabled_terminal(): void
    {
        ...
        ($this->handler)(new ActivateTerminal($terminalId->toNative()));

        $activated = array_values(array_filter(
            $this->eventStore->loadEvents($terminalId->toNative()),
            fn ($e) => $e instanceof TerminalActivated
        ));

        $this->assertCount(1, $activated);
    }

    public function test_refuses_to_activate_a_decommissioned_terminal(): void
    {
        $terminalId = $this->registerTerminal();
        $disable = new DisableTerminalHandler($this->terminalRepository);
        $disable(new DisableTerminal($terminalId->toNative()));
        ...
        $this->expectExceptionMessage('Cannot activate a decommissioned terminal');
