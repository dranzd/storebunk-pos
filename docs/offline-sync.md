...
StartNewOrderOfflineHandler::__invoke()
  ...
  └─ 8. idempotencyRegistry->markAsProcessed($commandId, $purpose)
...
SyncOrderOnlineHandler::__invoke()
  ...
  └─ 7. idempotencyRegistry->markAsProcessed($commandId, $purpose)
```
...
3. Handlers check `IdempotencyRegistry::hasBeenProcessed($commandId, $purpose)` before executing, where `$purpose` says what the command does (`purposeFor(messageName, orderId)`). Already processed for that same purpose → the handler returns immediately; known for DIFFERENT work → refused, because absorbing it would report success for work never done.
4. After successful processing, the handler calls `IdempotencyRegistry::markAsProcessed($commandId, $purpose)`.
...
1. **`IdempotencyRegistry`** — has this exact command id already done THIS
   work? Each id is recorded with what it did, built by
   `IdempotencyRegistry::purposeFor()`, so a redelivery is absorbed while the
