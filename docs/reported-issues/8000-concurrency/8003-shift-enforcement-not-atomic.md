...
**Follow-up hardening (2026-08-18, fourth and fifth review rounds):** the local review broke the third round's fixes twice, each time by running the same two commands in the opposite order, and the root cause was the same both times: a CHECK where a CLAIM was needed. A transfer whose target is already the shift's operator claims nothing, and an "does this shift id exist" check holds nothing, so a rival that completed in the gap did not stop the loser from writing on top. The fix is optimistic concurrency, which `ShiftRepositoryInterface::store()` had accepted since day one while no handler ever passed it: every shift command whose decision depends on what it read (open — against version 0 — assign, unassign, close, force-close) now stores against the version it read. A cash drop passes one too — it refuses a closed shift, so its decision depends on the state it read. That guard is only as strong as the store behind it, which the interface now documents, and the demo's file store enforces the same rule where it can actually be seen — inside the write lock, against the current file, refusing an event whose version is already taken. Without that, two demo processes that both read version N each appended their own N+1 and left two events claiming one version: a stream no replay can order, which wedged every later command on that shift permanently and which reconciliation reported as healthy. Reconciliation's refusal of an already-broken history also moved off the seeding path — seeding runs at every bootstrap, so refusing there made the CLI unstartable, including the very commands that recover it. Validated by deterministic in-process interleavings (`InterleavingShiftRepository`, hooking both load and store), by `FileShiftSlotReservationTest` pinning the permissive-seed / refusing-reconcile split, by `FileEventStoreTest` pinning the store-level refusal (taken version refused, refused append leaves the history untouched, a multi-event command is all-or-nothing, a malformed history reported as such rather than as a race), and by real concurrent demo processes for racing cash drops and racing assigns — exactly one winner each, stream well-formed, shift still closable afterwards.
