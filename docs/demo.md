...
One corruption cannot be repaired in place: a history where two events of the same aggregate claim the same version, which an older build could write when two commands raced. While any history is in that state, every `shift` and `session` command refuses, printing the aggregate and `state clear` as the remedy. That is deliberately blunt: those commands' guards — is this shift free of active sessions, is this terminal occupied — are answered by replaying the history, and a guard answered from a history that cannot be ordered is a guard that silently passes. The broken stream is NOT hidden from the projection, so a shift whose history is unorderable keeps holding its terminal in the projection; that is the safe failure, because it is visible and `state clear` clears it. Slots are not re-seeded from such a history at all — an ambiguous order is not a basis for deciding what is occupied. `terminal` commands still run, unless the unorderable stream is a terminal's own, in which case they too fail loudly with the same remedy. `state show` and `state clear` always work: `state show` only reads the id file and never replays events, and `state clear` is handled before anything is loaded.
