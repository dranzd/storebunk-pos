...
One corruption cannot be repaired in place: a history where two events of the same aggregate claim the same version, which an older build could write when two commands raced. While any history is in that state, every `shift` and `session` command refuses, printing the aggregate and `state clear` as the remedy. That is deliberately blunt: those commands' guards — is this shift free of active sessions, is this terminal occupied — are answered by replaying the history, and a guard answered from a history that cannot be ordered is a guard that silently passes. The broken stream is NOT hidden from the projection, so a shift whose history is unorderable keeps holding its terminal; that is the safe failure, because it is visible and `state clear` clears it. `terminal` commands and `state show` still run, unless the unorderable stream is a terminal's own, in which case they too fail loudly with the same remedy. `state clear` always works — it is handled before anything is loaded.
